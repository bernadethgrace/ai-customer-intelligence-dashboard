...
# Ganti import dari Ollama ke Groq
...
@st.cache_data
def load_data():
    # Sesuaikan path jika berbeda saat deploy
    df = pd.read_csv("feature_store/sample_feature_store.csv")

    numeric_cols = [
        "yearly_income", "credit_score", "savings_ratio",
        "avg_monthly_inflow", "avg_monthly_outflow", "spend_income_ratio",
        "fraud_ratio", "travel_score", "digital_score",
        "night_transaction_ratio", "merchant_diversity_score",
        "fraud_transaction_count", "avg_fraud_amount",
        "days_since_last_transaction", "max_transactions_per_hour",
        "weekend_transaction_ratio", "essential_spend_ratio",
        "dining_score", "luxury_score", "current_age",
        "avg_monthly_transactions", "avg_active_days_per_month",
...
# =========================================
# AI ENGINE (MODIFIED FOR CHAT MODEL)
# =========================================
...
"""
    ...
    try:
        # ChatGroq menggunakan invoke, hasilnya objek message
        response_obj = llm.invoke(prompt)
        response = response_obj.content # Ambil konten teksnya
        ...
        return {
            ...
            "personas": ["Data Processing Error"],
            "recommendations": ["Check API connection", "Verify data consistency"]
        }
...
# Sisanya (Visualisasi, UI, Sidebar) tetap sama...
# [ ... Potongan Kode UI kamu yang lain ... ]
# =========================================
# BUILD RAW TABLE, SIDEBAR, ETC.
# =========================================

# (Pastikan memanggil llm.invoke(chat_prompt).content pada bagian chatbot di bawah)
